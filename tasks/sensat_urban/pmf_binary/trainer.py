...
class Trainer(object):
    ...
    def _initDataloader(self):
        ...
        if self.settings.distributed:
            ...
            train_loader = torch.utils.data.DataLoader(
                ...
                num_workers=self.settings.n_threads,
                shuffle=True,
                drop_last=True)  # 这里修改，把shuffle设置为False，默认为True会报错
    ...
    def _backward(self, loss):
        ...
        for param in self.model.parameters():
            if param.grad is not None:
                param.grad[torch.isnan(param.grad)] = 0.0  # 增加应对梯度爆炸的手段
    ...
    def _computeClassifyLoss(self, pred, label, label_mask, signal):
        ...
        loss_foc = loss_foc + loss_dc  # focal loss + dice loss 作为新的focal loss
    ...
    def run(self, epoch, mode="Train"):
        ...
        for i, (input_feature, input_label) in enumerate(dataloader):
            ...
            if mode == "Train":
                ...
                loss_lov, loss_foc = self._computeClassifyLoss(
                    pred=lidar_pred, label=input_label, label_mask=label_mask, signal=1)
                ...
                loss_lov_cam, loss_foc_cam = self._computeClassifyLoss(
                    pred=camera_pred, label=input_label, label_mask=label_mask, signal=1)
            ...
            if self.recorder is not None:
                ...
                log_str += "LR {:0.5f} Loss {:0.4f} mAcc {:0.4f} mIOU {:0.4F} Recall {:0.4f} Entropy {:0.4f} 1IOU {:0.4F}  2IOU {:0.4F} 3IOU {:0.4F} 4IOU {:0.4F} 5IOU {:0.4F} 6IOU {:0.4F} 7IOU {:0.4F} 8IOU {:0.4F} 9IOU {:0.4F} 10IOU {:0.4F} 11IOU {:0.4F} 12IOU {:0.4F} 13IOU {:0.4F}".format(
                    lr, loss.item(), mean_acc.item(), mean_iou.item(), mean_recall.item(), entropy_meter.avg, class_iou[1], class_iou[2], class_iou[3], class_iou[4], class_iou[5], class_iou[6], class_iou[7], class_iou[8], class_iou[9], class_iou[10], class_iou[11], class_iou[12], class_iou[13])
        ...
        if self.recorder is not None:
            ...
            if epoch % self.settings.print_frequency == 0 and self.settings.dataset != "nuScenes":
                ...
                if camera_pred is not None:
                    for i in range(1, camera_pred.size(1)):
                        self.recorder.tensorboard.add_image(
                            "{}_RGBPred_cls_{:02d}_{}".format(mode, i - 1, self.mapped_cls_name[i - 1]),
                            camera_pred[0, i:i + 1].cpu(), epoch)

                for i in range(1, lidar_pred.size(1)):
                    self.recorder.tensorboard.add_image(
                        "{}_Pred_cls_{:02d}_{}".format(mode, i - 1, self.mapped_cls_name[i - 1]),
                        lidar_pred[0, i:i + 1].cpu(), epoch)
            ...
            log_str = ">>> {} Loss {:0.4f} mAcc {:0.4f} mIOU {:0.4F} Recall {:0.4f} 1IOU {:0.4F} 2IOU {:0.4F} 3IOU {:0.4F} 4IOU {:0.4F} 5IOU {:0.4F} 6IOU {:0.4F} 7IOU {:0.4F} 8IOU {:0.4F} 9IOU {:0.4F} 10IOU {:0.4F} 11IOU {:0.4F} 12IOU {:0.4F} 13IOU {:0.4F}".format(
                mode, loss_meter.avg, mean_acc.item(), mean_iou.item(), mean_recall.item(), class_iou[1].item(), class_iou[2].item(), class_iou[3].item(), class_iou[4].item(), class_iou[5].item(), class_iou[6].item(), class_iou[7].item(), class_iou[8].item(), class_iou[9].item(), class_iou[10].item(), class_iou[11].item(), class_iou[12].item(), class_iou[13].item())
            self.recorder.logger.info(log_str)
        ...
        result_metrics = {
            ...
            "1iou": class_iou[1].item(),
            "2iou": class_iou[2].item(),
            "3iou": class_iou[3].item(),
            "4iou": class_iou[4].item(),
            "5iou": class_iou[5].item(),
            "6iou": class_iou[6].item(),
            "7iou": class_iou[7].item(),
            "8iou": class_iou[8].item(),
            "9iou": class_iou[9].item(),
            "10iou": class_iou[10].item(),
            "11iou": class_iou[11].item(),
            "12iou": class_iou[12].item(),
            "13iou": class_iou[13].item(),
            "last": 0
        }
