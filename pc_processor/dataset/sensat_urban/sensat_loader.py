...
class SensatLoader(Dataset):
    def __init__(self, dataset, img_h=800, img_w=800, n_samples_split=200):
        ...
        if self.split == "train":
            self.aug_pos_0 = transforms.Compose([
            ...
            self.aug_pos_1 = transforms.Compose([
                # transforms.RandomCrop(
                #     size=(self.img_h*2, self.img_w*2)
                # ),
                transforms.RandomRotation(360),
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            while valid_percent < 0.1:
                if all_map.shape[1] > self.img_h*2 and all_map.shape[2] > self.img_w*2:
                    tmp_map = self.aug_pos_0(all_map)
                else:
                    tmp_map = self.aug_pos_1(all_map)
