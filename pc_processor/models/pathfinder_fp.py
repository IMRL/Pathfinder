...
class LearnableBiasnn(nn.Module):  ## 卷积偏置
...
class LearnableBiastt(nn.Module):  # transformer偏置
    def __init__(self, out_chn):
...
class RPReLU(nn.Module):
    def __init__(self, out_chn, toc=1):  # toc:transformer or cnn
        super(RPReLU, self).__init__()
        self.toc = toc
        if self.toc == 1:
            self.move1 = LearnableBiasnn(out_chn)  # move1和move2都是可学习偏置
...
#     if linear:  # 对线性层：逐行计算均值和方差
#         # 计算缩放因子：取输入张量每一行的绝对值均值
...
class Token_for_Attention(nn.Module):
    def __init__(self, dim, window_size=8):
        ...
        self.mlp = Mlp(dim, hidden_features=dim)  # 多层感知机
        # 初始化融合参数a1和a2
...
class BHViTSelfOutput(nn.Module):
    ...
    def __init__(self, embed_dims) -> None:
        super().__init__()
        self.dense = nn.Linear(embed_dims, embed_dims)
        self.dropout = nn.Dropout()
...
class Block(nn.Module):
    def __init__(self, embed_dims, mlp_ratio, toa, drop_path=0.):  # toa: token_mix or attention
        super().__init__()
...
class Embedding(nn.Module):
    ...
    def __init__(self, in_chans=3, out_chans=32, norm_layer=nn.BatchNorm2d, use_norm=True):
        super().__init__()
        self.proj = nn.Conv2d(in_chans, out_chans, kernel_size=3, padding=1, stride=2,
                              bias=True)
        self.norm = norm_layer(out_chans) if use_norm else nn.Identity()  # 归一化层，BN
...
class ResidualBasedFusionBlock(nn.Module):
    def __init__(self, pcd_channels, img_channels):
        ...
        self.fuse_conv = nn.Conv2d(pcd_channels + img_channels, pcd_channels, 3, 1, 1)
        ...
        self.attention_part2 = nn.Conv2d(pcd_channels, pcd_channels, 3, 1, 1)
        self.norm3 = nn.BatchNorm2d(pcd_channels)
        # self.rprelu3 = RPReLU(pcd_channels)
...
class ASPP_binary(nn.Module):
    ...
    def forward(self, x):
        image_features = F.interpolate(self.mean(x), size=x.shape[2:],
                                       mode='nearest')
        ...
        net = (
                          image_features + x + atrous_block1 + atrous_block6 + atrous_block12 + atrous_block18) / 6
        return net
...
class UpBlock_1(nn.Module):
    def __init__(self, in_filters, out_filters, dropout_rate, drop_out=True):
        ...
        self.conv2 = nn.Conv2d(out_filters, out_filters, (3, 3), 1, 2, 2)
        self.act2 = RPReLU(out_filters)
        self.bn2 = nn.BatchNorm2d(out_filters)

        self.move3 = LearnableBias(out_filters)
        self.conv3 = nn.Conv2d(out_filters, out_filters, (2, 2), 1, 1, 2)
        self.act3 = RPReLU(out_filters)
...
class UpBlock_2(nn.Module):
    def __init__(self, in_filters, out_filters, dropout_rate, drop_out=True):
        ...
        self.pool = nn.AvgPool1d(2, 2)
        self.conv1 = nn.Conv2d(in_filters // 4 + in_filters, out_filters, (3, 3), 1, 1)
        self.bn1 = nn.BatchNorm2d(out_filters)
        self.act1 = nn.LeakyReLU()
        self.move2 = LearnableBias(out_filters)
        self.conv2 = nn.Conv2d(out_filters, out_filters, (3, 3), 1, 2, 2)
...
class SalsaNextFusion(nn.Module):
    def __init__(self, in_channels=8, nclasses=20, base_channels=32, img_feature_channels=[], softmax=True):
        ...
        self.upBlock1 = UpBlock_1(2 * 8 * base_channels, 8 * base_channels, self.dropout_ratio)
        self.upBlock2 = UpBlock_2(8 * base_channels, 4 * base_channels, self.dropout_ratio)
        self.upBlock3 = UpBlock_2(4 * base_channels, 2 * base_channels, self.dropout_ratio)
        self.upBlock4 = UpBlock_2(2 * base_channels, base_channels, self.dropout_ratio, drop_out=False)
