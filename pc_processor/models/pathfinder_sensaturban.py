...
class token_mixer(nn.Module):
    def __init__(self, in_chn,dilation1=1,dilation2=3,dilation3=5, kernel_size=3, stride=1, padding='same'):
        ...
        self.norm = nn.BatchNorm2d(in_chn) # BN层
    ...
    def forward(self, x):
        ...
        x1 = F.conv2d(x, real_weights,  stride=self.stride, padding=self.padding,dilation=self.dilation1,groups=self.shape[0])
        x2 = F.conv2d(x, real_weights2, stride=self.stride, padding=self.padding,dilation=self.dilation2,groups=self.shape[0])
        x3 = F.conv2d(x, real_weights3, stride=self.stride, padding=self.padding,dilation=self.dilation3,groups=self.shape[0])
        x = self.norm((1.0-self.a-self.b).expand_as(x1)*x1+self.a.expand_as(x2)*x2+self.b.expand_as(x3)*x3)
        x = self.act(x)
        return x

class Token_for_Attention(nn.Module):
    def __init__(self, dim, window_size=10):
    ...
    def Merge_token(self,x):
        ...
        merge_token = (1.0-self.a1-self.a2).expand_as(merge_token_mlp)*merge_token_mlp+self.a1.expand_as(merge_token_max)*merge_token_max+self.a2.expand_as(merge_token_avage)*merge_token_avage
        return merge_token
    def forward(self, x):
...
class Attention(nn.Module):
    def __init__(self, dim, window_size=10, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(self, embed_dims, mlp_ratio, toa, window_size=10, qkv_bias=True, drop=0., attn_drop=0.,
...
class CFI_ViT(nn.Module):
    ...
    def __init__(self, layers, mlp_ratios, embed_dims, toa, in_chans=3, num_classes=1000, depth=12,
        ...
        self.blocks3 = nn.Sequential(*[
            Block(embed_dims[2], mlp_ratio=mlp_ratios[2], window_size=10,
        ...
        self.blocks4 = nn.Sequential(*[
            Block(embed_dims[3], mlp_ratio=mlp_ratios[3], window_size=10,
