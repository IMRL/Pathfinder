...
class token_mixer(nn.Module):
    def __init__(self, in_chn, dilation1=1, dilation2=3, dilation3=5, kernel_size=3, stride=1, padding='same'):
        ...
        self.scalef = nn.Parameter(torch.ones([1, in_chn, 1, 1]), requires_grad=True) # 缩放因子
        self.stride = stride
        ...
        self.act1 = RPReLU(in_chn, toc=1) # nn形式，输入格式为BCHW
    ...
    def forward(self, x):
        ...
        x = act_quant_fn(x) * self.scalef # 二值神经网络的梯度传播（正传&反传）* 全精度的尺度因子
...
class BHViTSelfAttention(nn.Module):
    def __init__(self, dim) -> None:
        ...
        self.token_FA = Token_for_Attention(dim=dim, window_size=10)
...
class ResidualBasedFusionBlock(nn.Module):
    def __init__(self, pcd_channels, img_channels):
        ...
        # QuantizeConv2d: inchn outchn kernel_size=3 stride=1 padding=1 dilation
...
class SalsaNextFusion(nn.Module):
    def __init__(self, in_channels=8, nclasses=20, base_channels=32, img_feature_channels=[], softmax=True):
        ...
        # Embedding 32 Bit
        self.downCntx = ResContextBlock(in_channels, base_channels) # in_channels: 8, base_channels: 32 为什么变成了48？
        # Embedding 1 Bit
    ...
    def forward(self, x, img_feature=[]):
        ...
        down0c, down0b = self.resBlock1(downCntx)  # 64，fbl
        down0c = self.fusionblock_1(down0c, img_feature[0].detach())

        down1c, down1b = self.resBlock2(down0c)  # 128，
        down1c = self.fusionblock_2(down1c, img_feature[1].detach())

        down2c, down2b = self.resBlock3(down1c)  # 256，
        down2c = self.fusionblock_3(down2c, img_feature[2].detach())

        down3c, down3b = self.resBlock4(down2c)  # 512，
...
class RGBDecoder(nn.Module):
    def __init__(self, in_channels=[], nclasses=14, base_channels=64):
        ...
        self.conv = nn.Conv2d(base_channels, nclasses, kernel_size=1, padding=0)
...
class Pathfinder_Sensaturban_Binary(nn.Module):
    def __init__(self, pcd_channels=5, img_channels=3, nclasses=20, base_channels=32,
                 checkpoint_file=[]):
        super(Pathfinder_Sensaturban_Binary, self).__init__()
