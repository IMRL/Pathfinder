...
class BHViTSelfAttention(nn.Module):
    def __init__(self, dim) -> None:
        ...
        self.rpreluq = RPReLU(dim, toc=2) # tt格式的bias
...
class LearnableBias(nn.Module):
    def __init__(self, out_chn):
        ...
        self.bias = nn.Parameter(torch.zeros(1, out_chn, 1, 1), requires_grad=True)  # BCHW 用于卷积的偏置，每个通道一个偏置
...
class ResContextBlock(nn.Module):
    ...
    def forward(self, x):
        # shortcut分支，先经过1x1卷积，再经过LeakyReLU激活函数
...
class UpBlock_2(nn.Module):
    def __init__(self, in_filters, out_filters, dropout_rate, drop_out=True):
        ...
        self.pool = nn.AvgPool1d(2, 2)
        self.conv1 = QuantizeConv2d(in_filters // 4 + in_filters, out_filters, (3, 3), 1, 1)
        self.bn1 = nn.BatchNorm2d(out_filters)
        self.act1 = nn.LeakyReLU()
        self.move2 = LearnableBias(out_filters)
        self.conv2 = QuantizeConv2d(out_filters, out_filters, (3, 3), 1, 2, 2)
